  ...
  <div class="total">
    ...
    <div class="content" ref="anchorContent" id="anchorContent">
            ...
            <div class="talk-list">
              ...
              <div style="padding: 1rem">
               <!--  <span
                  ...
                  :key="type"
                  @click="handleClick(type, item.id)"
                >
      ...
      <div class="word-nav" id="word">
        ...
        <span class="word-nav-two more-btn" @click="$router.push('checkWord')"
          >更多>></span
        >
      </div>
...
export default {
  ...
  methods: {
   ...
   async handleCommentClick( postid) {
      ...
      if (this.commentText != "收起评论") {
        this.showComment = true;
        this.commentText = "收起评论";
        let result=await commentService.getCommentByPostId(postid);
       this.commentList= result.data.data.rows.commentList
       console.log(result);
      } else {
        this.showComment = false;
        this.commentText = `${this.commentList.length}条评论`;
      }
    },
       /* async handleClick(type, postid) {
      console.log(type);
      if (type == "message" && this.actions[2].text != "收起评论") {
       ...
       console.log(result);
        ...
        this.showComment = false;
        this.actions[2].text = `${this.commentList.length}条评论`;
      }
    }, */
