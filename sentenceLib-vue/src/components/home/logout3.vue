  ...
  >
      ...
      <a-table 
	    ...
	    :rowSelection="{selectedRowKeys: selectedRowKeys, onChange: onSelectChange}">
	      ...
	      <span slot="action" slot-scope="text,record">
				<a href="javascript:void(0);" @click="openDetailModal(record)">详情</a>
    ...
    <div class="test" style="width: 80%; margin: 0 auto; display: flex">
      <div class="notice" style="width: 50%">
        ...
        >
          <a-list-item slot="renderItem" slot-scope="item, index" @click="$router.push('noticeDetail')">
            <a-card >
            <div slot="title">
            ...
            <a class="dated" @click="$router.push('noticeDetail')">
              ...
              <div :style-="{'display':'inline','font-weight':'bold'}"><strong>{{item.title}}</strong></div>
              </div>
      ...
      >
<!-- 
        <div class="kuangkaung" @click="$router.push('checkWord')" v-if="this.userRoles.includes('帮语审核员')">
        <img :src="cihui"  />
          词汇库
        </div>
        <div class="kuangkaung" @click="$router.push('workerWordPage')" v-else-if="this.userRoles.includes('帮语工者')">
        <img :src="cihui"  />
          词汇库
        </div> -->
        <div class="kuangkaung" @click="$router.push('wordPage')" >
        <img :src="cihui"  />
          词汇库
        </div>
       
        <!-- <div class="kuangkaung" @click="$router.push('checkSentence')" v-if="this.userRoles.includes('帮语审核员')">
          <img :src="yuliao" />
          句型库
        </div>     
       <div class="kuangkaung" @click="$router.push('workerSentencePage')" v-else-if="this.userRoles.includes('帮语工者')">
          <img :src="yuliao" />
          句型库
        </div> -->
        <div class="kuangkaung" @click="$router.push('sentencePage')">
          <img :src="yuliao" />
          句型库
        </div>
...
<!--     <a-list :grid="{ gutter: 16, column: 4 }" :data-source="dongtaiData">
    ...
    <div :style="{'background':'#123456','height':'100px','text-align':'center','color':'white'}">
     
<span>020-62793083 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><span>xiaoxiao@scut.mail.edu.cn</span>
<p>广州市华南理工大学B8实验室</p>
    </div>
  </div>
</template>
<script>
import { Form, Like } from "@icon-park/vue";
      ...
      gonggaodata: [
        {
          ...
          content:" XbotPark将3周的创业体验计划和6周的创业实战计划并行,参加其中任意一类活动的同学,都将直观地体验产品设计的过程，学会挖掘用户潜在需求",
...
bandService.getUserRoles({
        ...
        },
        {
          onSuccess: (model, fModel, json) => {
            console.log(json);
            this.userRoles=json.data.userRoles.rows.map((item)=>item.roleName);//对数组中的每一个元素处理，只截取它的某个属性
            if(this.userRoles!=null){console.log(this.userRoles);}
            console.log("用户角色是空的")
...
bandService.getUserRoles({
        ...
        },
        {
          onSuccess: (model, fModel, json) => {
            console.log(json);
            this.userRoles=json.data.userRoles.rows.map((item)=>item.roleName);
            if(this.userRoles!=null){console.log(this.userRoles);}
            console.log("用户角色是空的")
    ...
    onCardClick: function () {
     // this.$router.push("/appcase/" + id);登录才能查看
     alert("您还未登录不能查看剧本具体内容，请登录！");
    },
    onMoreClick: function () {
     // this.$router.push({ name: "AppCaseList" });登录才能查看
     alert("您还未登录不能查看更多剧本，请登录！");
    },
