...
const Utils = {
  ...
  copyProperties(from, to, options) {
    ...
    options = options || { $alone: true };
    // 需要复制一下存储一些环境参数，防止影响原来的参数对象
    if (!options.$alone) {
      options = Object.assign({ $alone: true }, options);
    }
    // 记录复制的顶层from对象
    if (!options.from) {
      options.from = from;
    ...
    if (!options.beforeMerge) {
      options.beforeMerge = (key, from, fromVal, to, toValue) => {
    ...
    for (let property in from) {
      ...
      // 非强制覆盖情况下, 由合并回调（默认判断值是否相等）确定是否继续合并，显式返回false就不做处理了
      if (!options.force && options.beforeMerge(property, from, fromVal, to, to[property]) === false) {
        continue;
      }
      // 如果已经遇到过这个值了, 则直接覆盖并跳过
      ...
      // 如果有验证回调则进行验证，如果不能处理，则直接覆盖并跳过
      if (options.mergeAccept && !options.mergeAccept(fromVal, property, from, to)) {
        to[property] = fromVal;
        continue;
      }
      // 如果是数组, 合并后赋值
      if (Utils.isArray(fromVal)) {
        ...
        for (let item of fromVal) {
          ...
          if (Utils.isObject(item)) {
            // 如果已经遇到过这个值了, 则后面写入覆盖并跳过
            if (options._meeted.indexOf(item) != -1) {
              continue;
            }
            let copyItem = {};
  ...
  getAccept(types) {
    ...
    const result = [];
    types.forEach((item) => {
      let suffix = item.toLowerCase();
      result.push(...match[suffix]);
    });
    return result.join(",");
  },
  /**
