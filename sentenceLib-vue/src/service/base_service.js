import { getBaseUrl, getBaseContext, search, ajax, toast, error } from "common/env";
...
export default class BaseService {
    ...
    this.CACHE_RULES = {
      ...
      MULTI: {
        ...
        get: (params, placeholders, args) => {
          ...
          if (!objs || objs.length != placeholders["objIds"].length) {
            /* 数量不一致当做没缓存 */
            return {
              objs: [],
              singleResult: false,
        ...
        remove: (params, placeholders, args) => {
          var removeObjIds = placeholders["removeObjId"] ? [placeholders["removeObjId"]] : placeholders["removeObjIds"];
          const objs = this._getObjsInCache(removeObjIds, placeholders["objType"], params, placeholders);
          if (!objs || !objs.length) {
            return {
              objs: [],
              singleResult: false,
      ...
      PARENT_CHILD: {
        identifyKeys: ["parentObjType", "subObjType", "parentObjId", "subObjIdCacheKey", "|removeObjId|removeSubObjId"],
        get: (params, placeholders, args) => {
          let parentObj = this._getObjInCache(
            placeholders["parentObjId"],
            placeholders["parentObjType"],
            params,
            placeholders
          );
          if (!parentObj) {
            parentObj = this._generateUnfetchObjAndCache(
              placeholders["parentObjId"],
              placeholders["parentObjType"],
              params,
              placeholders
        ...
        set: (params, placeholders, args) => {
          let cacheable = this.CACHE_RULES.SINGLE.set(
            params,
            {
          ...
          _setCacheSubObjIdProp(placeholders, parentObj, _getCacheId(obj, params, placeholders));
        ...
        remove: (params, placeholders, args) => {
          const cacheable = this.CACHE_RULES.SINGLE.remove(
            params,
            {
              objId: placeholders["removeObjId"] || placeholders["removeSubObjId"],
              objType: placeholders["subObjType"],
            },
            args
          );
          ...
          _updatingCache(params, placeholders, obj, parentObj);
          this._putObjInCache(parentObj, placeholders["parentObjType"], params, placeholders, true);
          return {
            objs: cacheable.objs,
            parentObj: parentObj,
            singleResult: true,
      ...
      PARENT_CHILDREN: {
        identifyKeys: [
          ...
          "parentObjId",
          "subObjIdsCacheKey",
          "|removeObjId|removeObjIds|removeSubObjId|removeSubObjIds",
        ],
        get: (params, placeholders, args) => {
          let parentObj = this._getObjInCache(
            placeholders["parentObjId"],
        ...
        set: (params, placeholders, args) => {
          let cachable = this.CACHE_RULES.MULTI.set(
            params,
            {
              objType: placeholders["subObjType"],
              innerCall: true,
          ...
          for (let obj of objs) {
            ...
            // 已存在不处理
            if (cachedSubObjIds.indexOf(_getCacheId(obj, params, placeholders)) != -1) {
              continue;
            }
            // 创建时插入到前面
            if (placeholders._handling == HANDLING__CREATE) {
              cachedSubObjIds = [_getCacheId(obj, params, placeholders)].concat(cachedSubObjIds);
            } else {
              // 否则直接拼接到后面
              cachedSubObjIds.push(_getCacheId(obj, params, placeholders));
            }
        ...
        remove: (params, placeholders, args) => {
          const cacheable = this.CACHE_RULES.MULTI.remove(
            params,
            {
              removeObjId: placeholders["removeObjId"] || placeholders["removeSubObjId"],
              removeObjIds: placeholders["removeObjIds"] || placeholders["removeSubObjIds"],
              objType: placeholders["subObjType"],
            },
            args
          );
          ...
          for (let obj of cacheable.objs) {
            ...
            let index = cachedSubObjIds.indexOf(_getCacheId(obj, params, placeholders));
            if (index != -1) {
  ...
  async queryTemplate(params, callbacks, placeholders, task) {
    ...
    const _loadCache4Inner = () => {
      return new Promise((resolve) => {
        ...
        // 设置缓存读取成功的回调
        placeholders._cacheReadyFn = (cache, hasSync) => {
    ...
    if (preventAjax) {
      ...
      const result = await _loadCache4Inner();
      // 基本回调 - end
    ...
    if (!preventCache) {
      if (fromCache) {
        let cacheable = this.queryFromCache(params, placeholders, callbacks);
        if (cacheable.objs.length) {
          return success4Inner(cacheable, { $fromCache: true });
        }
      } else {
  ...
  queryFromCache(params, placeholders, callbacks) {
    ...
    // 排序放到搜索后执行，因为搜索结果会改变原来顺序
    let cacheable = this._handleCacheByRule(params, placeholders, "get", { withoutSort: cacheSearches ? true : false });
    if (cacheSearches) {
      cacheable.objs = _sortingCache(
        params,
        placeholders,
        _searchCache(cacheable.objs, cacheSearches, cacheSearches.strict),
        cacheable.parentObj
      );
    }
   ...
   * @param {Object} params 请求参数
   * @param {Object} placeholders 占位参数
   * @param {Boolean} force 强制覆盖，即不与旧缓存数据合并
   * @return {Model Array} 缓存的对象模型集合
   */
  _putObjsInCache(objs, objType, params, placeholders, force) {
    const result = [];
    for (let obj of objs) {
      const cachedObj = this._putObjInCache(obj, objType, params, placeholders, force);
      if (cachedObj) {
        result.push(cachedObj);
      }
    }
    return result;
  }
   ...
   * @return {Model Array} 缓存的对象模型集合
   */
  _putObjInCache(obj, objType, params, placeholders, force) {
    const cacheObj = new objType(obj, Utils.isInstance(obj, objType), placeholders);
    if (!Utils.isString(_getCacheId(cacheObj, params, placeholders))) {
      throw new Error("请保证模型的id为字符串类型");
    }
    if (params["preventCacheStore"] || placeholders["preventCacheStore"]) {
      return cacheObj;
    }
    const cacheOptions = _getCacheOptions(params, placeholders);
    const cacheManager = _getCacheManager(params, placeholders, objType);
    const cache = cacheManager.cacheOf(objType);
    _putCache(cache, cacheObj, cacheOptions, force);
    if (_isUnfetchObj(cacheObj)) {
      return cacheObj;
    }
    return this._getObjInCache(_getCacheId(cacheObj, params, placeholders), objType, params, placeholders);
  }
  ...
  async _handleDetailsFetch(params, callbacks, placeholders, json) {
    ...
    for (let copyKey of ["objId", "objIds", "objType", "model", "url", "ajaxParams"]) {
      if (!detailFetchPlaceholders.hasOwnProperty(copyKey) || !detailFetchPlaceholders[copyKey]) {
        continue;
      }
      queryPlaceholders[copyKey] = detailFetchPlaceholders[copyKey];
    }
    // 复制一些params参数中比较重要的内容
    const queryParams = {
      ...detailFetchPlaceholders["params"] /* 允许通过占位参数的内容指定 */,
      fromCache: detailFetchPlaceholders["fromCache"],
    };
    for (let copyKey of Config.SERIES_COPY_PARM_NAMES) {
      if (!params.hasOwnProperty(copyKey) || !params[copyKey]) {
        continue;
      }
      queryParams[copyKey] = params[copyKey];
    }
...
const _ajaxTemplate = async (params, callbacks, placeholders, task) => {
  ...
  const json = await ajax({
    ...
    abort: params["requestAbort"] || placeholders["requestAbort"],
    timeout: params["requestTimeout"] || placeholders["requestTimeout"],
    error: (response, err) => {
      // 内部定义回调 - 处理错误
      let ajaxErrorFn = placeholders["_ajaxErrorFn"];
      if (Utils.isFunc(ajaxErrorFn) && ajaxErrorFn(response, err)) {
        return;
      }
      // 如果有任务, 则标记任务已失败
      if (task) {
  ...
  if (Utils.isFunc(ajaxSuccessFn)) {
    ...
    if (handleResult !== false) {
      ...
      // 错误识别, 返回true说明被处理了, 顺序按全局通用 -> 外部指定 -> 错误标签 -> 默认处理
      if (!_handleError(json, "COMMON", callbacks, placeholders)) {
        callbacks.onFail(_getErrorMsgFromJson(json, placeholders["errorMsgPath"]) || placeholders["errorMsg"], json);
      } else if (Utils.isFunc(callbacks.onFail) && !callbacks.onFail.default) {
        callbacks.onFail(_getErrorMsgFromJson(json, placeholders["errorMsgPath"]) || placeholders["errorMsg"], json);
      } else if (!_handleError(json, placeholders["errorTag"], callbacks, placeholders)) {
        callbacks.onFail(_getErrorMsgFromJson(json, placeholders["errorMsgPath"]) || placeholders["errorMsg"], json);
      }
      // 根据重试回调是否存在来调用内部定义回调 - 重试
      let retryFn = placeholders["_retryFn"];
 ...
 * 判断是否是未加载对象
 * @param {Model} obj
 * @return {Boolean} 是否是未加载对象
 */
const _isUnfetchObj = (obj) => {
  return obj && obj.name == Config.CACHE_MISSED_NAME ? true : false;
};
...
const _getObjType = (placeholders) => {
  return (
    placeholders["objType"] ||
    placeholders["subObjType"] ||
...
const _getCacheOptions = (params, placeholders) => {
  ...
  cacheOptions.onReady = (cache, hasSync) => {
    const oldReadyFn = cacheOptions.onReady;
    if (Utils.isFunc(cacheReadyFn)) {
      oldReadyFn(cache, hasSync);
    }
    const cacheReadyFn = placeholders["_cacheReadyFn"];
    if (Utils.isFunc(cacheReadyFn)) {
      cacheReadyFn(cache, hasSync);
    }
  };
  cacheOptions.cacheIdOf = (obj) => {
    return _getCacheId(obj, params, placeholders);
  };
  return cacheOptions;
};
...
/**
 * 私有: 获取缓存模型的缓存ID
 * @param {Model} obj 缓存对象
 * @return {String} 缓存模型的缓存名称
 */
const _getCacheId = (obj, params, placeholders) => {
  if (params && Utils.isFunc(placeholders.cacheIdOf)) {
    return placeholders.cacheIdOf(obj, params, placeholders);
  }
  if (placeholders && Utils.isFunc(params.cacheIdOf)) {
    return params.cacheIdOf(obj, params, placeholders);
  }
  if (obj.constructor && Utils.isFunc(obj.constructor.cacheIdOf)) {
    return obj.constructor.cacheIdOf(obj, params, placeholders);
  }
  return obj.id;
};
 ...
 * @param {Boolean} force 强制覆盖，即不与旧缓存数据合并
 */
const _putCache = (cache, obj, options, force) => {
  const oldObj = cache.get((options && options.cacheIdOf(obj)) || obj.id, options);
  if (!force && oldObj) {
    Utils.copyProperties(obj, oldObj, Config.CACHE_COPY_OPTIONS);
    cache.cache((options && options.cacheIdOf(oldObj)) || oldObj.id, oldObj, options);
    return;
  }
  cache.cache((options && options.cacheIdOf(obj)) || obj.id, obj, options);
};
...
const _searchCache = (searchObjs, searches, strict) => {
  ...
  for (let key in searches) {
    if (!searches.hasOwnProperty(key) || ["or", "and"].indexOf(key) != -1) {
      continue;
    }
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result && searchResults.indexOf(result) == -1) {
  ...
  if (Utils.isObject(searches.and)) {
    return _searchCache(searchResults, searches.and, strict);
  } else if (Utils.isObject(searches.or)) {
    let results = _searchCache(searchResults, searches.or, strict);
    for (let i = 0; i < results.length; i++) {
      let result = results[i];
      if (result && searchResults.indexOf(result) == -1) {
        searchResults.push(result);
...
const _setCacheSubObjIdsProp = (placeholders, parentObj, newVal, obj) => {
  _setCacheProp(placeholders["subObjIdsCacheKey"], parentObj, Utils.isArray(newVal) ? newVal : [], obj);
};
...
const _pagingCache = (objs, pagination, callbacks) => {
  ...
  pagination.cachedTotal = objs.length;
  pagination.pagingCache = true;
  _setPagination(pagination, null, callbacks);
  pagination.pagingCache = false;
  pagination.$pagingEnd = false; /* 总是设为false */
...
const _setPagination = (pagination, ajaxParams, callbacks) => {
  ...
  delete pagination.command; // 使用一次消耗一次，防止缓存和真实ajax请求的分页处理冲突
  ...
  if (!pagination.pagingCache) {
    let currentTime = Date.now();
    if (oldPagination.$lastPagingTime && currentTime - oldPagination.$lastPagingTime <= Config.MIN_PAGING_INTERVEL) {
      return false;
    }
  ...
  // 上面是对旧的分页对象的数据进行修改的，这里把修改后的更新到外部传入的分页对象中
...
const _handleDefaultErrorCallbacks = (callbacks) => {
  ...
  if (!callbacks.onError) {
    callbacks.onError = (response, errorMsg) => {
      if (errorMsg) {
        console.error("请求异常: ", errorMsg, response);
      }
      error(response);
    };
